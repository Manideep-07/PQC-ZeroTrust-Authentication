...
async def test_invalid_hex_payload(client: httpx.AsyncClient, base_url: str):
    """
    Negative test: sending malformed hex to /auth/handshake must return HTTP 400.
    Security property: server rejects unparseable inputs without crashing.
    """
    payload = {
        "client_id": "neg_test_hex",
        "kyber_public_key": "ZZZNOTVALIDHEX",
        "challenge": "ZZZNOTVALIDHEX",
        "device_hash": "x",
        "os_identifier": "x",
    }
    resp = await client.post(f"{base_url}/auth/handshake", json=payload, timeout=10.0)
    if resp.status_code == 400:
        print("[neg_test] Invalid hex payload: PASS — server returned 400")
        return True
    else:
        print(f"[neg_test] Invalid hex payload: FAIL — server returned {resp.status_code}")
        return False


async def test_missing_session_token(client: httpx.AsyncClient, base_url: str):
    """
    Negative test: sending a non-existent session token to /secure/data must return HTTP 401.
    Security property: server rejects unauthenticated data requests.
    """
    from crypto.aes_gcm import AESGCMWrapper
    import secrets as _secrets
    fake_key = _secrets.token_bytes(32)
    aes = AESGCMWrapper(fake_key)
    nonce, ct = aes.encrypt(b"ghost message")
    payload = {
    ...
    resp = await client.post(f"{base_url}/secure/data", json=payload, timeout=10.0)
    if resp.status_code == 401:
        print("[neg_test] Missing session token: PASS — server returned 401")
        return True
    else:
        print(f"[neg_test] Missing session token: FAIL — server returned {resp.status_code}")
        return False


async def test_forged_verify_signature(client: httpx.AsyncClient, base_url: str):
    """
    Negative test: sending a randomly-signed verify challenge must return HTTP 401.
    Security property: /auth/verify rejects client identity proofs from wrong keypair.
    """
    from crypto.kyber import KyberWrapper
    from crypto.dilithium import DilithiumWrapper
    import secrets as _secrets
    ...
    if r.status_code != 200:
        print(f"[neg_test] Forged verify sig: SKIP (handshake failed {r.status_code})")
        return True
    data = r.json()
    verify_challenge = data.get("verify_challenge", "")
    if not verify_challenge:
        print("[neg_test] Forged verify sig: SKIP (no verify_challenge)")
        return True
    # Use a fresh, unrelated keypair to sign — this is the forgery
    fake_pk, _ = dilithium.generate_keypair()
    fake_sig = dilithium.sign(bytes.fromhex(verify_challenge))
    verify_payload = {
        "session_token": data["session_token"],
    ...
    if rv.status_code == 401:
        print("[neg_test] Forged verify signature: PASS — server returned 401")
        return True
    else:
        print(f"[neg_test] Forged verify signature: FAIL — server returned {rv.status_code}")
        return False
...
async def main():
    ...
    async with httpx.AsyncClient() as _neg_client:
        neg1 = await test_invalid_hex_payload(_neg_client, base_url)
        neg2 = await test_missing_session_token(_neg_client, base_url)
        neg3 = await test_forged_verify_signature(_neg_client, base_url)
    neg_pass = neg1 and neg2 and neg3
    ...
    print(f"Negative Security Tests: {'PASS' if neg_pass else 'FAIL'}")
